...
**Иерархия:** Регионы можно организовать в дерево. Например, страна → город:
...
### Региональная схема для Беларуси

В сети MeshCore Беларуси используется простая двухуровневая структура со всеми областными центрами:
...
**Выберите код вашего города** из списка выше для настройки устройств ниже.
...
```bash
region            # Показать все настроенные regions
```
